			...
			)
{
	...
	*newlinePos = '\0';
	...
	buffer->correct = (uint8_t)0U;		/* zunächst keine Antworten als korrekt markieren */
	currentAnswer = 0;
	while(currentAnswer < NUM_ANSWERS)
	{
		...
		*newlinePos = '\0';	/* Zeilenvorschub löschen */
		...
		{
			strcpy(buffer->answers[currentAnswer], answerBuffer+2);
			buffer->correct |= (uint8_t)(1U << currentAnswer);
			++currentAnswer;
		}
		else
		{
			...
			if(currentAnswer == 0 && !strncmp(answerBuffer, timeoutPrefix, sizeof(timeoutPrefix)-1))
			{
				...
				if(timeoutTmp == 0 || timeoutTmp > 120)	/* Timeout zu klein oder zu groß */
				{
					res->status = PARSE_INVALID_TIMEOUT;
					return 0;
				}
				buffer->timeout = (uint8_t)timeoutTmp;
			}
